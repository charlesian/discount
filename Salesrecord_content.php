    ...
    <div class="card">
        <div class="card-header p-2">
            <ul class="nav nav-pills">
                ...
                <?php if ($define_tab == 1): ?>
                <li class="nav-item"><a class="nav-link " href="#activity" data-toggle="tab">Records</a></li>
                <li class="nav-item"><a class="nav-link active" href="#timeline" data-toggle="tab">Set Membership
                        Discounts</a></li>
                <?php else: ?>
                <li class="nav-item"><a class="nav-link active" href="#activity" data-toggle="tab">Records</a></li>
                                                ...
                                                ?>
                                                    <div class="col-sm-12">
                                                        <table class="table">
                                                            ...
                                                            <tbody>
                                                                <?php 
$view_query = mysqli_query($conn, "SELECT * FROM tbl_sales WHERE store_id = $acc_id ");
while ($row = mysqli_fetch_assoc($view_query)) {
    $userid = $row["customer_id"];
    $user_query = mysqli_query($conn, "SELECT * FROM tbl_account WHERE acc_id = $userid ");
while ($row2 = mysqli_fetch_assoc($user_query)) {
    $username = $row2["acc_name"];
}
$salesid = $row["sales_id"];
	$finaltotal = $row["finaltotal"];
    $membership = $row["membership"];
	echo "<tr><td>".$username."</td><td style='text-transform:capitalize'>".$membership."</td><td>".$finaltotal."</td><td><a href='?salesid=".$salesid."'><button class='btn btn-info'>View</button><button class='btn btn-danger'>Delete</button></td></tr>";
}
...
?>
    ...
    ?>
                                                    ...
                                                    <div class="row">
                                                    <div class="col-sm-12">
                                                    <h3>Sales #<?php echo $_GET['salesid'];?></h3>
                                                    </div>
													<div class="col-sm-8">
...
?>
											...
											<table class="table productsale">
                                               ...
                                               $salesid = $_GET['salesid'];
                                                $view_query = mysqli_query($conn, "SELECT * FROM tbl_sales WHERE store_id = $acc_id AND sales_id= $salesid");
                                                while ($row = mysqli_fetch_assoc($view_query)) {
                                                    $userid = $row["customer_id"];
                                                    $user_query = mysqli_query($conn, "SELECT * FROM tbl_account WHERE acc_id = $userid ");
                                                while ($row2 = mysqli_fetch_assoc($user_query)) {
                                                ...
                                                foreach ($items as $key => $value) {
                                                   ...
                                                   $price= "{$value->amount}";
                                                    $totalinfo=$price*$qty;
                                                    $amount=$price*$qty;
                                                   echo "<tr><td style='display:none'>".$productid."</td><td>".$productname."</td><td><input class='ind-qty' mainid='".$productid."' value='" .$qty."' id='".$amount."' info='".$totalinfo."'></td><td id='price-".$productid."'>".$price."</td><td id='amount-".$productid."' class='dataamount'>".$amount."</td><td style='color:red;cursor:pointer' class='remove' info='".$totalinfo."'>X</td></tr>";
...
var oldtotal = $(this).attr("id");
var value = $(this).val();
var totaly = $(".total").text();
var mainid = $(this).attr("mainid");
var price = $("#price-"+mainid).text();

var oldqualvalue = $(this).attr("info");

var amount = parseFloat(price)*parseFloat(value);

if (value != "" && value != "0") {
    if (totaly > oldqualvalue) {
        var totalminus = parseInt(totaly) -
            parseInt(
                oldqualvalue);
    } else {
        var totalminus = parseInt(
                oldqualvalue) -
            parseInt(totaly);
    }
                                            ...
                                            <table class="table">
												<thead>
													<tr>
														...
														<th>
															<h4><span class="total"><?php echo  $finaltotal;?></span></h4>
														</th>
													</tr>
												</thead>
                        ...
                        success: function (result) {
                            ...
                            $.ajax({
                                ...
                                success: function (results) {
                                    ...
                                    var final = parseFloat(totals) * parseFloat(
                                        "0." + results);
                                    var final = parseFloat(totals) - parseFloat(
                                        final);
                                    $(".topay").text(final);
                            ...
                            $.ajax({
                                ...
                                success: function (result) {
									...
									$(".productsale tbody").append("<tr><td style='display:none'>"+id+"</td><td>" + product +
										"</td><td><input class='ind-qty' maini='"+id+"' value='" + qty +
										"' id='" + result + "' info='" + total + "'></td><td>" +
										result +
										"</td><td id='amount-"+id+"'>" + 
										result*qty +
